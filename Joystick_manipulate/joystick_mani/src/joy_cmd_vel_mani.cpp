...
void Joy_cmd_vel_mani::operate()
{
  ...
  crt_grip+=SENSITIVITY *grip_close;
  crt_grip+=SENSITIVITY *grip_open;
  ...
  crt_arm_z=MAXtoMIN(crt_arm_z,5);
  crt_arm_x=MAXtoMIN(crt_arm_x,5);
  crt_arm_y=MAXtoMIN(crt_arm_y,5);
  crt_hand =MAXtoMIN(crt_hand,18); 
  crt_grip =MAXtoMIN(crt_grip,18);

  // 현재 값이 180.0 ~ -180.0 사이에 있도록 함.
  ...
  dmx.mo5=(int)crt_grip*11.377 + 2047;
  dmx.mo4=(int)crt_hand*11.377+2047;
